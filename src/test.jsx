...
const ModelAr = () => {
  ...
  const [modelState , StateOfModel] = useState(null);
  const contentImgVdo1 = useSelector((state) => state.sideBarContentReducer.contentImgVdo);

  const [IdFromControlsAttach ,IdGetFromControlsAttach] = useState(null);
  // UseEffect Using ---------------------------------------------------------------------------------------->
  ...
  useEffect(() => {
    ...
    axios.get(API.BASE_URL + "scene_data_by_project/" + proId + "/")
      .then((response) => {
        setModelViewState(response.data.scenes[0].FeaturedtrackerOption);
      }).catch((err) => {});
    ...
    const boxModal = () => {
      ...
      let cameraPersp,cameraOrtho,currentCamera,canvas,labelRenderer,texture1,geometry,material , loDManager;
      let scene,renderer,control,orbit,plane,Videomesh,mesh,helper,clock,cPointLable;
      ...
      function init() {
        ...
        move.addEventListener("click", () => {control.setMode('translate')})
        scale.addEventListener("click", () => {control.setMode('scale')})
        rotate.addEventListener("click", () => {control.setMode('rotate')})
        scene.add(control);
        // control._gizmo.showX = true;
        // control._gizmo.showY = true;
        // control._gizmo.showZ = true;
        ...
        axios.get(API.BASE_URL + "scene_details/" + id + "/")
          .then((responseProject) => {
            ...
            set2D3D(responseProject.data.data[0].Twod_3D);
        ...
        if (getImage) {
          for (let i = 0; i < getImage.length && getImage !== undefined; i++) {
            const imageId = getImage[i][0].image_id;
            ...
            mesh.userData.name = imageData+'-'+imageId;
          }
        }
        ...
        if (getVideo) {
          for (let i = 0; i < getVideo.length && getVideo !== undefined; i++) {
            ...
            const Videogeometry = new THREE.BoxGeometry(
              getVideo[i][0].video_transform.width,
              getVideo[i][0].video_transform.height,
              1
            );
            ...
            Videomesh.userData.name = getVideo[i][0].video_url+"-"+getVideo[i][0].video_id;
          }
        }
        ...
        if (get3Dmodel) {
          for (let i = 0;i < get3Dmodel.length && get3Dmodel !== undefined;i++) {
            ...
            loader.load(gltfContent, (gltf) => {
              ...
              loadedObject.userData.name = get3Dmodel[i][0].file_url+"-"+get3Dmodel[i][0].threed_id;
            });
          }
        }


        // States for Text Featres ----------------------------------------------------------s-------------------------------------->
        ...
        if (isObjectIntersecting) {
          ...
          firstIntersected.object.traverse((object3D) => {
            ...
            if (canExecuteCallback) {
              ...
              var Url = object3D.userData.name;
              let idget
              if (Url) {
                var matched = Url.match(/\d+$/);
                idget = matched ? parseInt(matched[0]) : null;
                console.log(idget, "Yes Got");
              }
              let timer;
              ...
              control.addEventListener("change", function () {
                ...
                timer = setTimeout(function () {
                  ...
                  if (idget !== undefined) {
                    console.log(idget, "Yes, I got this");
      
                    // Update the position using API calls
                    axios.put(API.BASE_URL + "image_transform/" + idget + "/", {
                    ...
                    axios.put(API.BASE_URL + "video_transform/" + idget + "/", {
                      position_x: position.x,
                      position_y: position.y,
                      position_d: position.z,
                    })
                ...
                }, 4000); // Reduced the delay to 1000ms (1 second)
              });
            }
          });
        }
